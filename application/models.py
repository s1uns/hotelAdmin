...
class Premise_Inhabitant(models.Model):
    ...
    def __str__(self):
        return '{inhabitant} entered the {premise} from {checkIn} till {checkOut}'.format(inhabitant=self.inhabitant.__str__, premise=self.premise.__str__,  checkIn=self.checkIn_date, checkOut=self.checkOut_date)